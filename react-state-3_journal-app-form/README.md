...
### GOTTA CHECK IF THE ENTRIES ARE CORRECTLY PASSED DOWN, CHECKLIST:

- Prop in Entries Section
- Correctly passing from app/EntriesSection via prop
- Console.log something (NO NEED - since everything is appearing, it must be working, no error message either)
...
- Write a function `handleSubmit` and pass it as the `onSubmit` attribute of the `form` JSX tag. ### THINK I GOT THIS RIGHT
...
### STILL HAVE TO CONNECT CREATED FORM TO OTHER COMPONENTS VIA PARENT (SEE odAddEntry)
